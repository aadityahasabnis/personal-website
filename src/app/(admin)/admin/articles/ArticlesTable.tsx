...
import { deleteArticle, toggleArticleFeatured, toggleArticlePublished } from '@/server/actions/articles';
...
export function ArticlesTable({ articles, topics }: IArticlesTableProps): React.ReactElement {
    const table = useAdminTable({
        tableKey: 'admin-articles',
        data: articles,
        keyExtractor: (a) => a.slug,
        searchFn: (article, query) =>
            article.title.toLowerCase().includes(query) || article.description?.toLowerCase().includes(query) || article.tags?.some((tag) => tag.toLowerCase().includes(query)) || false,
    });
    ...
    const handleDelete = useCallback((article: IArticle) => table.optimisticDelete(article.slug, () => deleteArticle(article.topicSlug, article.slug)), [table]);
    ...
    return (
        <div className='space-y-6'>
            <TableSearch placeholder='Search articles...' onSearch={table.setSearchQuery} filters={tableFilters} onFilterChange={table.setFilters} activeFiltersCount={table.activeFiltersCount} />
            <DataTable
