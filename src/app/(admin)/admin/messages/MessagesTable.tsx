...
import { Mail, Calendar, Archive, Eye, Check, X, Trash2 } from 'lucide-react';
...
export function MessagesTable({ messages }: IMessagesTableProps): React.ReactElement {
    ...
    const getRowActions = (msg: SerializedMessage): IDataTableAction[] => {
        ...
        actions.push(
            createDeleteAction(
                () => table.optimisticDelete(msg._id, () => deleteMessage(msg._id)),
    ...
    const TypeBadge = ({ type }: { type: string }) => {
        ...
        return (
            <span className={cn('px-2 py-0.5 rounded-full text-xs font-medium capitalize', colors[type] || colors.general)}>
                {type}
            </span>
        );
    };
    ...
    const columns: IDataTableColumn<SerializedMessage>[] = [
        {
            ...
            accessor: (msg) => (
                <div className="min-w-0">
                    <p className={cn('font-medium line-clamp-1', !msg.read && !msg.archived && 'font-semibold')}>
                        {msg.name}
                    </p>
        ...
        },
        {
            ...
            accessor: (msg) => (
                ...
                >
                    <p className={cn('font-medium line-clamp-1 hover:underline', !msg.read && !msg.archived && 'font-semibold')}>
                        {msg.subject}
                    </p>
    ...
    const bulkActions: IBulkActionNew[] = [
        {
            ...
            action: (ids) => table.optimisticBulkUpdate(
                ids,
                (m) => ({ ...m, read: true }),
                async () => { await Promise.all(ids.map((id) => markMessageRead(id))); }
            ),
        },
        {
            id: 'archive',
            ...
            action: (ids) => table.optimisticBulkUpdate(
                ...
                async () => { await Promise.all(ids.map((id) => archiveMessage(id))); }
            ),
        },
