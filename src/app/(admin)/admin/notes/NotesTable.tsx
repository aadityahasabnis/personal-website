...
import { deleteNote, toggleNoteFeatured, toggleNotePublished } from '@/server/actions/notes';
...
export function NotesTable({ notes }: INotesTableProps): React.ReactElement {
    const table = useAdminTable({
        tableKey: 'admin-notes',
        data: notes,
        keyExtractor: (note) => note.slug,
        searchFn: (note, query) => note.title.toLowerCase().includes(query) || note.description?.toLowerCase().includes(query) || note.tags?.some((tag) => tag.toLowerCase().includes(query)) || false,
    });
    ...
    const getRowActions = (note: INote) => [
        ...
        createDeleteAction(() => table.optimisticDelete(note.slug, () => deleteNote(note.slug)), `"${note.title}"`),
    ];
    ...
    const columns: IDataTableColumn<INote>[] = [
        {
            ...
            accessor: (note) => (
                <div className='min-w-0 max-w-md'>
                    <Link href={`/admin/notes/${note.slug}/edit`} className='font-medium hover:underline hover:text-foreground line-clamp-1 block'>
                        {note.title}
                    </Link>
                    {note.description && <p className='mt-0.5 text-sm text-muted-foreground line-clamp-1'>{note.description}</p>}
                </div>
            ),
            width: '350px',
        },
    ...
    const bulkActions: IBulkActionNew[] = [
        createBulkPublishAction((ids) =>
            table.optimisticBulkUpdate(
                ...
                (n) => ({ ...n, published: true }),
                async () => {
                    await Promise.all(ids.filter((id) => !table.items.find((n) => n.slug === id)?.published).map((id) => toggleNotePublished(id)));
                },
            ),
        ),
        ...
        createBulkFeatureAction((ids) =>
            table.optimisticBulkUpdate(
                ...
                (n) => ({ ...n, featured: true }),
                async () => {
                    await Promise.all(ids.filter((id) => !table.items.find((n) => n.slug === id)?.featured).map((id) => toggleNoteFeatured(id)));
                },
            ),
        ),
                ...
                emptyState={
                    <div className='p-12 text-center'>
                        ...
                        <p className='mt-2 text-muted-foreground'>
                            {table.searchQuery || table.activeFiltersCount > 0 ? 'Try adjusting your search or filters' : 'Create your first note to get started'}
                        </p>
