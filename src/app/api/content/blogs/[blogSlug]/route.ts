import { NextResponse } from 'next/server';

import { getPublishedBlogByPath } from '@/server/new/public/content/blog';

import { toHttp } from '../_shared';

export const runtime = 'nodejs';
export const dynamic = 'force-dynamic';

export const GET = async (
    _request: Request,
    context: { params: Promise<{ blogSlug: string }> }
): Promise<NextResponse> => {
    const { blogSlug } = await context.params;
    return toHttp(await getPublishedBlogByPath(blogSlug));
};

export const OPTIONS = (): NextResponse => {
    return NextResponse.json({
        endpoint: '/api/content/blogs/:blogSlug',
        methods: ['GET'],
        querySchema: {
            blogSlug: 'string required in path',
        },
        responseExamples: {
            get200: {
                success: true,
                status: 200,
                data: {
                    id: '65f1502cdc9bc503f8d7c001',
                    slug: 'scaling-read-heavy-services',
                    title: 'Scaling Read-Heavy Services',
                },
            },
        },
    });
};
