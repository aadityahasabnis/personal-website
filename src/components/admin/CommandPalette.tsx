...
import { BarChart3, BookOpen, FileText, FolderKanban, Home, Layers, Mail, Plus, Search, Settings, type LucideIcon } from 'lucide-react';
                ...
                <div className='mx-4 overflow-hidden rounded-xl border bg-background shadow-2xl'>
                    ...
                    <div className='flex items-center gap-3 border-b px-4'>
                        ...
                        <input
                            ...
                            autoFocus
                        />
                        <kbd className='hidden rounded bg-muted px-2 py-1 text-xs text-muted-foreground sm:inline-block'>ESC</kbd>
                    </div>
                        ...
                        {filteredCommands.length === 0 ? (
                            ...
                            <>
                                ...
                                {groupedCommands.navigation.length > 0 && (
                                    <div className='mb-2'>
                                        ...
                                        {groupedCommands.navigation.map((cmd) => {
                                            ...
                                            return <CommandItem key={cmd.id} command={cmd} isSelected={globalIndex === selectedIndex} onClick={() => executeCommand(cmd)} />;
                                        })}
                                    </div>
                                )}
                                ...
                                {groupedCommands.settings.length > 0 && (
                                    <div className='mb-2'>
                                        <div className='px-2 py-1.5 text-xs font-semibold text-muted-foreground'>Settings</div>
...
function CommandItem({ command, isSelected, onClick }: CommandItemProps) {
    ...
    return (
        <button
            onClick={onClick}
            className={cn('flex w-full items-center gap-3 rounded-lg px-3 py-2 text-left transition-colors', isSelected ? 'bg-primary text-primary-foreground' : 'hover:bg-muted')}
        >
            <div className={cn('flex h-8 w-8 items-center justify-center rounded-md', isSelected ? 'bg-primary-foreground/10' : 'bg-muted')}>
                <Icon className='h-4 w-4' />
            </div>
            <div className='flex-1 overflow-hidden'>
                <div className='truncate text-sm font-medium'>{command.label}</div>
                {command.description && <div className={cn('truncate text-xs', isSelected ? 'text-primary-foreground/70' : 'text-muted-foreground')}>{command.description}</div>}
            </div>
        </button>
    );
}
