 ...
 * />
 */
export const SearchFilter = ({ placeholder = 'Search...', searchValue, onSearchChange, filterGroups = [], onFilterChange, className }: ISearchFilterProps): React.ReactElement => {
    ...
    return (
        <div className={cn('flex items-center gap-2', className)}>
            ...
            <div className='relative flex-1'>
                ...
                <Input type='search' placeholder={placeholder} value={searchValue} onChange={(e) => onSearchChange(e.target.value)} className='pl-9 pr-9' />
                {searchValue && (
                    ...
                    <DropdownMenuTrigger asChild>
                        <Button variant='outline' className='relative'>
                            ...
                            {activeFiltersCount > 0 && (
                                <span className='ml-2 flex h-5 w-5 items-center justify-center rounded-full bg-primary text-xs text-primary-foreground'>{activeFiltersCount}</span>
                            )}
                        </Button>
                    </DropdownMenuTrigger>
                        ...
                        {filterGroups.map((group, groupIndex) => (
                            <div key={group.id}>
                                ...
                                {group.options.map((option) => (
                                    <DropdownMenuCheckboxItem key={option.value} checked={option.checked} onCheckedChange={(checked) => onFilterChange?.(group.id, option.value, checked)}>
                                        {option.label}
