...
export const ImageGallery = ({ onSelect, folder = 'portfolio/content', className }: ImageGalleryProps) => {
    ...
    const filteredImages = images.filter((img) => img.filename.toLowerCase().includes(searchQuery.toLowerCase()));
            ...
            <div className={cn('p-6', className)}>
                <div className='flex items-start gap-3 p-4 rounded-lg bg-destructive/10 text-destructive'>
                    ...
                    <div className='space-y-1'>
                        ...
                        <button onClick={fetchImages} className='text-sm underline hover:no-underline mt-2'>
                            Try again
                        </button>
                    </div>
                </div>
        ...
        <div className={cn('space-y-4', className)}>
            ...
            <div className='grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-3 max-h-[500px] overflow-y-auto pr-2'>
                {filteredImages.map((image) => (
                    <ImageCard key={image.publicId} image={image} selected={selectedImage?.publicId === image.publicId} onSelect={() => handleSelect(image)} />
                ))}
            </div>

            {filteredImages.length === 0 && searchQuery && (
                <div className='text-center py-8'>
                    <p className='text-sm text-muted-foreground'>No images found matching "{searchQuery}"</p>
                </div>
            )}
        </div>
    );
...
const ImageCard = ({ image, selected, onSelect }: { image: GalleryImage; selected: boolean; onSelect: () => void }) => {
    return (
        <button
            type='button'
            onClick={onSelect}
            className={cn(
                ...
                selected ? 'border-green-500 ring-2 ring-green-500' : 'border-transparent hover:border-primary/50',
            )}
        >
            {/* Image */}
            <div className='aspect-square bg-muted relative'>
                ...
                <div className='absolute inset-0 bg-black/60 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center'>
                    <div className='text-white text-center space-y-1 p-2'>
                        ...
                        <p className='text-xs opacity-80'>{(image.bytes / 1024).toFixed(0)}KB</p>
                    </div>
                </div>
            ...
            <div className='p-2 bg-card'>
                <p className='text-xs truncate text-center'>{image.filename}</p>
            </div>
        </button>
    );
};
