...
import { CodeBlock as CodeBlockPrimitive, type CodeBlockProps as CodeBlockPropsPrimitive } from '@/components/animate-ui/primitives/animate/code-block';
...
function Code({ className, code, ...props }: CodeProps) {
    return (
        <CodeProvider value={{ code }}>
            <div className={cn('relative flex flex-col overflow-hidden border bg-accent/50 rounded-lg', className)} {...props} />
        </CodeProvider>
    );
}
...
function CodeHeader({ className, children, icon: Icon, copyButton = false, ...props }: CodeHeaderProps) {
    ...
    return (
        <div className={cn('bg-accent shrink-0 gap-x-2 border-b border-border/75 dark:border-border/50 text-sm flex text-muted-foreground items-center px-4 w-full h-10', className)} {...props}>
            {Icon && <Icon className='size-4' />}
            {children}
            {copyButton && <CopyButton content={code} size='xs' variant='ghost' className='ml-auto w-auto h-auto p-2 -mr-2' />}
        </div>
    );
}
