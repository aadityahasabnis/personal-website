...
function RippleButton({ className, variant, size, ...props }: RippleButtonProps) {
    return <RippleButtonPrimitive className={cn(buttonVariants({ variant, size, className }), rippleButtonVariants[variant as keyof typeof rippleButtonVariants])} {...props} />;
}
