 ...
 */
export const ContentCard = ({ href, title, description, tags, date, readingTime, variant = 'default', index = 0, className }: IContentCardProps) => {
    const isFeatured = variant === 'featured';
    const isCompact = variant === 'compact';

    return (
        <motion.article initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5, delay: index * 0.1 }} className={cn('group relative', className)}>
            <Link href={href} className='block'>
                ...
                >
                    ...
                    {tags && tags.length > 0 && (
                        <div className='flex flex-wrap gap-2 mb-4'>
                            {tags.slice(0, 3).map((tag) => (
                                <span key={tag} className='text-xs font-medium uppercase tracking-wider text-[var(--accent)]'>
                                    {tag}
                                </span>
                            ))}
                        </div>
                    ...
                    {/* Description */}
                    {description && !isCompact && <p className={cn('mt-3 text-[var(--fg-muted)] line-clamp-2', isFeatured ? 'text-base md:text-lg' : 'text-sm')}>{description}</p>}
