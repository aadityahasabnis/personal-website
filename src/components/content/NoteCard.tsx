            ...
            <Link href={`/notes/${note.slug}`} className="block h-full">
                <div
                    className={cn(
                        ...
                        'hover:-translate-y-1',
                        'dark:hover:shadow-[0_0_40px_-10px_var(--accent)]'
                    )}
                >
                    {/* Icon Badge */}
                    <div className="mb-4 inline-flex items-center justify-center size-12 rounded-xl bg-gradient-to-br from-[var(--accent)]/10 to-[var(--accent)]/5 text-[var(--accent)] border border-[var(--accent)]/20">
                        <Lightbulb className="size-6" />
                    </div>

                    {/* Title */}
                    <h3 className="text-lg md:text-xl font-semibold text-[var(--fg)] mb-3 transition-colors group-hover:text-[var(--accent)] line-clamp-2">
                        {note.title}
                    </h3>
                    ...
                    {note.tags && note.tags.length > 0 && (
                        <div className="flex flex-wrap gap-2 mb-4">
                            {note.tags.slice(0, 3).map((tag) => (
                                <span
                                    key={tag}
                                    className="inline-flex items-center gap-1 px-2.5 py-1 rounded-md text-xs font-medium bg-[var(--accent)]/10 text-[var(--accent)] border border-[var(--accent)]/20"
                                >
                    ...
                    <div className="absolute top-6 right-6 opacity-0 group-hover:opacity-100 transition-all duration-300 transform group-hover:translate-x-1 group-hover:-translate-y-1">
                        <div className="size-8 rounded-full bg-[var(--accent)] flex items-center justify-center shadow-lg shadow-[var(--glow-color)]">
                            <ArrowUpRight className="size-4 text-white" />
                        </div>
                    </div>

                    {/* Subtle gradient overlay on hover */}
                    <div className="absolute inset-0 rounded-2xl bg-gradient-to-br from-[var(--accent)]/0 to-[var(--accent)]/0 group-hover:from-[var(--accent)]/[0.02] group-hover:to-[var(--accent)]/[0.05] transition-all duration-300 pointer-events-none" />
                </div>
            </Link>
        </motion.article>
    );
