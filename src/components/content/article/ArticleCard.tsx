...
const ArticleCard = ({ article, variant = 'default', className }: IArticleCardProps) => {
    ...
    return (
        ...
        >
            <Link href={`/articles/${article.slug}`} className='block h-full p-6'>
                ...
                {/* Title */}
                <h3 className={cn('font-semibold tracking-tight transition-colors', 'group-hover:text-primary', isFeatured ? 'text-2xl md:text-3xl' : isCompact ? 'text-base' : 'text-xl')}>
                    {article.title}
                </h3>

                {/* Description */}
                {article.description && !isCompact && <p className='mt-2 line-clamp-2 text-muted-foreground'>{article.description}</p>}
