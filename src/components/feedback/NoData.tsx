...
const NoData = (props: INoDataProps) => {
    const { description = "There's nothing here just yet.", className, mode = 'neutral', imgClassName = 'text-6xl', title } = props;

    const renderAction = () => {
        if (props.actionType === 'link') {
            return (
                <Link href={props.href} className='text-primary hover:underline font-medium'>
                    {props.actionMessage}
                </Link>
            );
        }
        if (props.actionType === 'action') {
            return (
                <button onClick={props.onAction} className='text-primary hover:underline font-medium'>
                    {props.actionMessage}
                </button>
            );
        }
    ...
    return (
        <div className={cn('flex flex-col items-center justify-center gap-4 p-8 min-h-[240px]', className)}>
            <span className={imgClassName}>{modeEmoji[mode]}</span>
            <div className='flex flex-col items-center text-center gap-2'>
                {title && <h5 className='text-h6 text-foreground uppercase font-medium'>{title}</h5>}
