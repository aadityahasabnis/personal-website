...
export const Scales = ({ orientation = 'diagonal', size = 10, className, color }: ScalesProps) => {
    ...
    return (
        <div
            className={cn('absolute inset-0 h-full w-full overflow-hidden', '[--pattern-scales:var(--color-neutral-950)]/10', 'dark:[--pattern-scales:var(--color-white)]/10', className)}
            style={
                {
                    '--scales-size': `${size}px`,
...
export const ScalesContainer = ({ children, orientation = 'diagonal', size = 10, className, containerClassName, color }: ScalesContainerProps) => {
    const scalesProps: ScalesProps = {
        orientation,
        size,
